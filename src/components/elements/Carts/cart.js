...
import style from "@/components/elements/Carts/index.module.css";
...
const isDisablebutton = ()=> {
  return carts.length === 0
    ...
    return(
      ...
      <div className={style["cart__Sales"]}>
        <div>
          <p style={{fontSize:"20px", marginRight:"30px"}}>Total Price :</p>
        </div>
        <div>
      <p style={{fontSize:"30px"}}>{formatTotal}</p>
        </div>
        </div>
        <div className={style["cart__Button"]}>
          <button onClick={()=>window.my_modal_1.showModal()} disabled={isDisablebutton()}>Payment</button>
          </div>
          </div>
  ...
  <form method="dialog" className="modal-box">
    ...
    <p className="py-1" style={{fontSize:"50px"}}>{formatTotal}</p>
    </div>
    <div className="modal-action">
      {/* if there is a button in form, it will close the modal */}
      <div className="modal-action" style={{display:"flex", justifyContent:"space-around", width:"100%"}}>
        <button className="btn btn-success" onClick={handlePaymentConfirmation}>Confirm Payment</button>
        <button className="btn btn-error" onClick={() => window.my_modal_1.close()}>Cancel Payment</button>
      </div>
    </div>
    </div>
  </form>
