    ...
    {carts.map((cart, index) =>{
      ...
      <tr key={index}>
        ...
        <div className={style["cart-item__action"]}>
            <button onClick={() => handleDecreseToCart(cart)} style={{backgroundColor:"red"}}>-</button>
            <p>{cart.quantity}</p>
            <button onClick={() => handleAddToCart(cart)} style={{backgroundColor:"greenyellow"}}>+</button>
            </div>
        </td>
        <td>{formattedPrice}</td>
        <td>{producttotal}</td> 
        <td>
        <button style={{color:"white"}} className="btn btn-warning btn-xs" onClick={()=> handleRemove(cart)}>Remove</button>
        </td>
      </tr>
            )
        })}
